...
class AdminDetailEarPhoneActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        productId = intent.getStringExtra("productphoneId") ?: ""

        // Tải thông tin sản phẩm từ Firebase bằng productmenId
        databaseReference =
            FirebaseDatabase.getInstance().reference.child("Product").child("Classify")
                .child("Ear_Phones").child(productId)

        databaseReference.addValueEventListener(object : ValueEventListener {
            @SuppressLint("SetTextI18n")
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    val product = snapshot.getValue(ProductEarPhonesAccessories::class.java)
                    if (product != null) {
                        ...
                        if (!isDestroyed) {
                            Glide.with(this@AdminDetailEarPhoneActivity)
    ...
    private fun saveImageToStorage(imageUri: Uri) {
        val storageReference =
            FirebaseStorage.getInstance().reference.child("admin_add_product/phones_accessories/$uid2/$productId.jpg")
        ...
        databaseReference.removeValue()
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Toast.makeText(
                        this@AdminDetailEarPhoneActivity,
